<template>
    <v-container id="sensors-page">
        <h1>Sensors</h1>
        <v-row v-if="!loading">
            <v-col cols="12" md="3" v-for="(sensor,i) in sensors" :key="i">
...
export default {
    ...
    mounted(){
        axios.get(process.env.VUE_APP_SENSOR_API_BASE + '/GetSensors')
             ...
             .then(() => { this.sensors.push({id:"123456-123345-123-123-987", name:"Coop Outdoors", type:"DHT22", heartbeatInterval:10, lastHeartbeat: (new Date()).toISOString() })} ) // console.log(response))
